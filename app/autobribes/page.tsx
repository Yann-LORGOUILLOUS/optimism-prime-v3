...
import { useReliquaryData, PoolDisplayData, RelicDisplayData } from '@/presentation/hooks/useReliquaryData';
...
const DISCLAIMER_STORAGE_KEY = 'opp_autobribes-warning-acknowledged';
const DISCLAIMER_TIMESTAMP_KEY = 'opp_autobribes-warning-acknowledged-timestamp';
const MEDIUM_ARTICLE_URL = 'https://medium.com/optimism-prime/introducing-autobribes-8fed520b9b60';
const TWITTER_URL = 'https://twitter.com/OptimismPrime_';
const OP_TOKEN_URL = 'https://optimistic.etherscan.io/token/0x4200000000000000000000000000000000000042';
const FEATURED_POOL_NAME = 'OP/VELO';
...
export default function AutobribesPage() {
  ...
  const { data, isLoading, error, refetch, loadAllRelics } = useReliquaryData(reliquaryVersion, 'autobribes');
  ...
  const { featuredPool, advancedPools, featuredPoolIndex } = useMemo(() => {
    ...
    const featured = data.pools.find(p => p.shortName.includes(FEATURED_POOL_NAME));
  ...
  return (
    <Card>
      <p className="text-white/80 text-lg text-center">
        Deposit your Velodrome LP tokens in the Autobribes Reliquary to earn a share of voting bribes
      </p>
      <p className="text-white/60 mt-2 text-center">
        ...
        {' '}or follow us on{' '}
        <a href={TWITTER_URL} target="_blank" rel="noopener noreferrer" className="text-red-400 hover:text-red-300 underline underline-offset-4 transition-colors">
          Twitter
        </a>
      </p>
    </Card>
...
function ReliquaryOverviewSection({ data, version, onRefresh, isRefreshing }: { 
  data: ReturnType<typeof useReliquaryData>['data'];
  version: ReliquaryVersion; 
  onRefresh: () => void;
    ...
    <Card>
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl md:text-3xl text-white" style={{ fontFamily: 'Transformers, sans-serif' }}>
          Autobribes for Velodrome V{version}
        </h2>
        <button 
          onClick={onRefresh} 
    ...
    <Card>
      ...
      <div className="bg-black/30 rounded-xl p-6 border border-white/5">
        <div className="flex items-center justify-between mb-6 px-2">
          ...
          >
            {showLevels ? 'Hide Levels' : `Show ${pool.levels.length} Levels`}
          </button>
        </div>
...
function UserRelicsSection({ data, visibleRelics }: {
  data: NonNullable<ReturnType<typeof useReliquaryData>['data']>;
      ...
      <div className="relative bg-linear-to-br from-black/90 via-black/80 to-black/90 backdrop-blur-xl border border-red-500/50 rounded-2xl p-8 max-w-lg mx-4 shadow-[0_0_60px_rgba(239,68,68,0.3)]">
        ...
        <div className="flex flex-col items-center gap-6 text-center">
          ...
          <p className="text-white/80 text-lg">
            The Autobribes code is unaudited. Use at your own risk.
          </p>
